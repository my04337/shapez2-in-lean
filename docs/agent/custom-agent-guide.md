...
├── lean-session-restorer.agent.md     # session restart / target 復元オーケストレータ
├── lean-simp-stabilizer.agent.md      # 1 行の simp → simp only 化
└── lean-theorem-investigator.agent.md # theorem target / def behavior の A→Z 調査
```
...
- 例: `lean-build-doctor.agent.md`, `lean-theorem-investigator.agent.md`
