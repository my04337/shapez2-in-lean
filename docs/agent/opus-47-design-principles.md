...
- **カタログ系**: ゴール形状 → タクティク／補題のマッピング表（例: `lean-proof-methods`）
- **ハンドブック系**: ツール（REPL / build / diag）の使い方リファレンス（例: `lean-tooling`, `lean-diagnostics`）
- **ガイド系**: 設計判断の指針（例: `s2il-proof-workflow`, `lean-simp-guide`）
