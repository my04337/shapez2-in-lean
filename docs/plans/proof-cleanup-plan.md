...
安定状態 (SettledState) の証明チェーンを `S2IL/Behavior/SettledState.lean` に実装中。
...
| SettledState 証明（ビルド通過） | - | **0 error, 4 sorry** |

---

## 1. 残課題一覧

### 1.1 安定状態 (Settled State) の証明

SettledState.lean に実装。Paint チェーンは完全証明済み。残りは sorry 3 件。

| # | 内容 | 状態 | 備考 |
|---|---|---|---|
| SS-1 | `IsSettled_paint` 着色で安定状態保存 | ✅ 完了 | paint チェーン全証明済み |
| SS-2 | `IsSettled_crystallize` 結晶化で安定状態保存 | 🔲 sorry | `floatingUnits_crystallize_eq` が未証明 |
| SS-3 | `IsSettled_rotateCW` 回転で安定状態保存 | 🔲 sorry | `floatingUnits_isEmpty_rotateCW` が未証明 |
| SS-4 | `gravity_IsSettled` 基盤定理 | 🔲 sorry | ★★★ |
| SS-5 | `SettledState.lean` への分離 | ✅ 完了 | 独立ファイルとして作成済み |
| SS-6 | `SettledShape` サブタイプの導入 | 🔲 将来 | SS-1~SS-4 完了後に検討 |
