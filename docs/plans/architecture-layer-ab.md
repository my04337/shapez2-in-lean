...
| 層 | 表現 | 用途 |
|---|---|---|
| **関係層** | `ClusterRel s := Relation.ReflTransGen (fun p q => IsBonded s p q)` | 論理的性質（到達可能性・対称性・回転持ち上げ）。Mathlib `Relation.ReflTransGen` をそのまま利用する |
| **集合層** | `clusterSet s start : Finset QuarterPos`（`noncomputable`）| 証明で使う正規表現。等変性は Finset 等式で述べる |
| **列挙層** | `clusterList s start : List QuarterPos` | `#eval` / 決定可能な計算で使う実装 |
| **橋渡し** | `clusterList_toFinset : (clusterList s start).toFinset = clusterSet s start`（1 本）| List 側と Finset 側の往復はこの 1 本経由でのみ行う |
...
3. `clusterList` の実装は `Kernel/Internal/ClusterImpl.lean` に閉じ込め、公開 API からは隠蔽する。

**等変性の 1 行系テンプレート**（Phase C 着手時のひな型）:

```lean
/-- クラスタ集合の CW 等変性。`Relation.ReflTransGen.lift` の直接適用で証明される。 -/
theorem clusterSet_rotateCW_comm (s : Shape) (start : QuarterPos) :
    clusterSet s.rotateCW start.rotateCW
      = (clusterSet s start).image QuarterPos.rotateCW := by
  ext p; simp [clusterSet, ClusterRel, Relation.ReflTransGen.lift, isBonded_rotateCW]
```
...
│   ├── Cluster.lean               # ClusterRel / clusterSet (Finset) / clusterList (List)
│   ├── Transform.lean             # rotateCW(=map Layer.rotateCW) / rotate180 / rotateCCW
│   └── Internal/
│       ├── ClusterImpl.lean       # clusterList の実装（Cluster.lean からのみ import）
│       ├── BondImpl.lean
