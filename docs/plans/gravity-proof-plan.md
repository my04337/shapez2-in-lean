...
| 定理 | 内容 | 主依存 |
|---|---|---|
| `dropOf_le_minLayer` | `dropOf s U ≤ U.minLayer`（layer 0 を割らない） | Drop.lean |
| `gravity.isSettled` | `IsSettled (gravity s)` | dropOf 構造、Collision |
| `gravity.of_isSettled` | `IsSettled s → gravity s = s` | floatingUnits = ∅ |
...
`Behavior.lean` の `gravity.of_isSettled` と `dropOf_le_minLayer` を実装。
`gravity.isSettled` は **Internal/Collision.lean の補題完成後** に着手（次段階）。

### Phase D-10D: 安定性主定理 — 2〜3 セッション想定（最大ヤマ）

`Internal/Collision.lean` を完成させ、`gravity.isSettled` を証明。

**最大の作業ノード**: 「fold で配置した結果のすべての非空象限が `IsGrounded`」の証明。
旧 `placeLDGroups_landing_groundingEdge_mono` に相当する内容だが、本モデルでは
`dropOf` を 1 回計算した固定写像として扱うため、cross-group 上書きが構造的に発生しない（衝突判定で排除される）。証明は `IsGrounded` の `Relation.ReflTransGen` 帰納法を「fold ステップごと」ではなく「最終 shape の全 path」で 1 度に行う。
...
- [ ] §4 の 9 件の `#guard` テストがすべて通過
- [ ] `S2IL/Operations/Gravity.lean` の 4 axiom が theorem 化
- [ ] `lake build` green、warning 0
- [ ] §5 のファイル構成が遵守されている（facade ≤ 150 行、一般 ≤ 300 行）
- [ ] §6 の補題チェーンが全て埋まっている（sorry 0）
- [ ] [architecture-layer-ab.md §1.4.4](architecture-layer-ab.md) と本書が実装と一致
- [ ] `S2IL/_agent/sorry-plan.json` の axioms[] から Gravity の 4 件が削除
