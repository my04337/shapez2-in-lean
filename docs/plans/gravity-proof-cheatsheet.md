...
| # | 定理 | 行 | 概要 | 真理値 | 難度 | 状態 |
|---|---|---|---|---|---|---|
| S-1 | `shouldProcessBefore_no_mutual` | ~~L5019~~ | floatingUnits 間で shouldProcessBefore 双方向 true 不可 | TRUE | ★★★ | ✅ 完了 |
| S-2 | `insertSorted_preserves_relative_order` | ~~L5253~~ | insertSorted が既存要素の相対順序保存 | TRUE | ★★ | ✅ 完了 |
| S-3 | ~~`sortFallingUnits_shouldProcessBefore_one_way_order`~~ | ~~L5274~~ | ~~one-way shouldProcessBefore ペアが sortFallingUnits で順序保存~~ | **FALSE** | - | ❌ **偽と判明、削除済み** |
| S-4a | `sortFallingUnits_inversion_is_tied` | L6194 | sortFallingUnits の反転ペアは tied (shouldProcessBefore 双方 false) | TRUE (計算検証) | ★★★ | ✅ **完了** (S-5 に依存) |
| S-4b | `sortFallingUnits_inversion_dir_disjoint` | L6234 | sortFallingUnits の反転ペアは方角素 | TRUE | ★★ | ✅ **完了** (S-4a に依存) |
| S-5 | `shouldProcessBefore_no_chain` | L5605 | floatingUnits 間で shouldProcessBefore 連鎖 (a→b→c) は不可 | **FALSE** | - | ❌ **偽と判明** |

### S-3 → S-4 の経緯

S-3 (`sortFallingUnits_shouldProcessBefore_one_way_order`) は **floatingUnits の制約下でも偽** と判明。
反例: `a=pin(2,NE), b=cluster[(4,NE),(4,SE)], c=pin(6,SE)` — spb の非推移三角形により
`sortFU([c,a,b])` で spb(a,b)=true でも a が b の後に来る。

代替として S-4 を **2段階** で構成:
- **S-4a** (`sortFallingUnits_inversion_is_tied`): sortFallingUnits の反転ペアが tied (shouldProcessBefore 双方 false) であることを主張 → **sorry**
- **S-4b** (`sortFallingUnits_inversion_dir_disjoint`): S-4a + `tied_no_shared_dir` + `floatingUnits_elem_positions_disjoint` から direction-disjoint を導出 → **✅ 完了**
...
```
process_rotate180 ← settle_foldl_eq ← sortFallingUnits_foldl_perm_input_eq
    ├── shouldProcessBefore_no_mutual (S-1)                               ✅ 完了
    ├── sortFallingUnits_inversion_dir_disjoint (S-4b)             ⚠️ 証明不健全 (S-5依存)
    │     ├── sortFallingUnits_inversion_is_tied (S-4a)            ⚠️ 証明不健全 (S-5依存)
    │     ├── tied_no_shared_dir                         ✅
    │     ├── tied_no_shared_dir_rev                     ✅
    │     └── floatingUnits_elem_positions_disjoint                 ✅
    └── foldl_eq_of_perm_tied_adj_comm                   ✅
```

> **S-1・S-2・S-4a・S-4b は完了**。S-3・S-5 は偽と判明。
> **S-5 (`shouldProcessBefore_no_chain`) は FALSE** — 証明チェーンの再構築が必要。
> ⚠️ S-4a/S-4b/sortFallingUnits_foldl_perm_input_eq の証明は S-5 に依存しており不健全。

### 各 sorry の詳細

**S-1: `shouldProcessBefore_no_mutual` — ✅ 完了**
...
**S-3: `sortFallingUnits_shouldProcessBefore_one_way_order` — ❌ 偽と判明、削除済み**
...
**S-4a: `sortFallingUnits_inversion_is_tied` — ✅ 完了 (L6194)**
- sortFU(l1) と sortFU(l2) の反転ペアが tied (shouldProcessBefore 双方 false)
- `sortFallingUnits_spb_order_preserving` (L6155) を使用して証明
  ...
  - 4-case 不変条件分析: (a=u,b=u)→矛盾, (a=u,b∈acc)→insertSorted_before_spb, (a∈acc,b=u)→shouldProcessBefore_no_chain+append, (a∈acc,b∈acc)→preserves_relative_order
- `shouldProcessBefore_no_chain` (S-5, sorry) に依存

**S-5: `shouldProcessBefore_no_chain` — ❌ 偽と判明 (2026-04-05)**
...
**S-4b: `sortFallingUnits_inversion_dir_disjoint` — ✅ 完了 (L6234)**
- sortFU(l1) と sortFU(l2) の反転ペアが direction-disjoint
- S-4a (`sortFallingUnits_inversion_is_tied`) + `floatingUnits_elem_positions_disjoint` + `tied_no_shared_dir` / `tied_no_shared_dir_rev` から導出
...
process_rotate180 (L6413)
  ...
  └─ settle_foldl_eq (L6307)               ✅ (Phase 1 + Phase 2)
       ...
       └─ sortFallingUnits_foldl_perm_input_eq (L6279)  ✅ (sorry × 1 内包)
            ├─ foldl_eq_of_perm_tied_adj_comm    ✅ バブルソート帰納法
            ├─ sortFallingUnits_inversion_dir_disjoint (L6234) ✅ (S-4b)
            │    ├─ sortFallingUnits_inversion_is_tied (L6194)  ✅ (S-4a)
            ...
            │    │         │    └─ shouldProcessBefore_no_chain (L5605)              ❌ sorry (S-5)
            │    │         └─ posIn_lt_of_decomposition (L6108)      ✅
            │    ├─ tied_no_shared_dir            ✅
            │    ├─ tied_no_shared_dir_rev        ✅
            │    └─ floatingUnits_elem_positions_disjoint    ✅
            ├─ shouldProcessBefore_no_mutual                     ✅ (S-1)
            └─ insertSorted_preserves_relative_order ✅ (S-2)
```
...
l_mid.Perm l2 を示し、`sortFallingUnits_foldl_perm_input_eq` を適用。
`sortFallingUnits_foldl_perm_input_eq` の本体は `foldl_eq_of_perm_tied_adj_comm` (バブルソート帰納法) を呼び出し、
h_tied_comm コールバック内で `sortFallingUnits_inversion_dir_disjoint` (S-4b) を使用。

---

## 6. sortFallingUnits_foldl_perm_input_eq の内部構造
...
`sortFallingUnits_inversion_dir_disjoint` (S-4b) を直接使用:
...
S-4a (`sortFallingUnits_inversion_is_tied`) は以下の 3 段階で証明可能:
...
3. 1+2 から、sortFallingUnits 出力間の inversion は tied のみ

**注意**: `sortFallingUnits_inversion_is_tied` の型シグネチャは一般の `l1.Perm l2` に対しては偽 (3L で 516 violations)。
...
| L5400-5600 | shouldProcessBefore_no_mutual | S-1 完了 | ✅ |
| L5600-5620 | shouldProcessBefore_no_chain | sorry (S-5) | ❌ |
...
| L6194-6234 | sortFallingUnits_inversion_is_tied | S-4a ✅ (S-5 に依存) | ✅ |
| L6234-6279 | sortFallingUnits_inversion_dir_disjoint | S-4b ✅ (S-4a に依存) | ✅ |
| L6279-6307 | sortFallingUnits_foldl_perm_input_eq | 本体完成 (S-4b に依存) | ✅ |
...
| sortFallingUnits_inversion_dir_disjoint は完全証明済み | sortFallingUnits_inversion_is_tied + tied_no_shared_dir から導出 (L6234) |
| **sortFallingUnits_inversion_is_tied は完全証明済み** | sortFallingUnits_spb_order_preserving + shouldProcessBefore_no_chain (sorry) から導出 (L6194) |
...
| sortFallingUnits_shouldProcessBefore_one_way_order は**一般には FALSE** | 3要素反例で確認 (check_sort_order.lean) |
| **sortFallingUnits_inversion_is_tied は一般 Perm で FALSE** | 3L 3色 8628 violations（Scratch/AllPermInvCheck.lean） |
| **h_ow 制約下では sortFallingUnits_inversion_is_tied は TRUE** | 3L 3色 全対で 0 violations。h_ow: spb(a,b)=true → a before b in both inputs |
...
| `sortFallingUnits_shouldProcessBefore_one_way_order` (一般リスト版) | 3要素反例: a={(0,NE)}, b={(1,NE),(0,SE)}, x={(1,SE)} で b が a より前に来る |
...
| `sortFallingUnits_inversion_is_tied` が一般 Perm で真 | 3L 3色 8628 violations（全順列）。l_mid/l2 の特定構造でのみ真。h_ow 制約下では 0 violations |
| `shouldProcessBefore_no_chain` (spb 2段連鎖不可) | 4層 Shape `"--------:P-------:--------:P-------:--------:P-------"` で 3 pin 連鎖。pin は canFormBond=false で常に独立。計算検証が 2-3L のみで不十分だった |
...
| `sortFallingUnits_foldl_perm_input_eq` (一般 Perm) | 反例2 で sortFU(fus).foldl ≠ sortFU(fus.reverse).foldl。方角共有ペアの順序が影響する |
...
#### S-4b: sortFallingUnits_inversion_dir_disjoint — ✅ 完了 (2026-04-05)

`sortFallingUnits_inversion_is_tied` (S-4a) + `tied_no_shared_dir` + `floatingUnits_elem_positions_disjoint` から完全導出。
数学的洞察「tied ↔ direction-disjoint for floatingUnits」を活用。

#### S-4a: sortFallingUnits_inversion_is_tied — ✅ 完了 (2026-04-04)
...
- `shouldProcessBefore_no_chain` (L5605, S-5 sorry): 連鎖不在
証明は `shouldProcessBefore_no_chain` (S-5) に依存する。

#### S-5: shouldProcessBefore_no_chain — ❌ 偽と判明 (2026-04-05)

`shouldProcessBefore_no_chain` は **FALSE** であることが判明。4層以上の形状で反例が成立。
...
- `sortFallingUnits_inversion_is_tied` — 一般 Perm で FALSE
- `sortFallingUnits_foldl_perm_input_eq` — 一般 Perm で FALSE
- `sortFallingUnits_inversion_dir_disjoint` — 一般 Perm で FALSE (is_tied に依存)
...
A. **r180 固有 Perm 制約で直接証明**: l_mid/l2 の入力反転が常に tied であることを直接利用。
   sortFallingUnits_inversion_is_tied を r180 特有の制約（one-way ペアが両入力で同順）で証明し直す。
   shouldProcessBefore_no_chain と sortFallingUnits_spb_order_preserving を不要にする。

B. **bubble sort 帰納法の Case 3 修正**: shouldProcessBefore_no_chain の代わりに
...
`shouldProcessBefore_no_chain` が FALSE であるため、以下の証明チェーンが不健全:

```
process_rotate180 ← settle_foldl_eq ← sortFallingUnits_foldl_perm_input_eq (❌ 一般 Perm で FALSE)
    ├── foldl_eq_of_perm_tied_adj_comm        ✅ (コールバック前提で健全)
    ├── sortFallingUnits_inversion_dir_disjoint          ❌ (is_tied 経由で不健全)
    │     └── sortFallingUnits_inversion_is_tied         ❌ (一般 Perm で FALSE)
    │           └── sortFallingUnits_spb_order_preserving  ❌ (FALSE)
    │                 └── foldl_insertSorted_preserves_spb_order  ❌ (Case 3 不健全)
    │                       └── shouldProcessBefore_no_chain   ❌❌ FALSE
    ├── shouldProcessBefore_no_mutual                          ✅ (独立、健全)
    └── insertSorted_preserves_relative_order  ✅ (独立、健全)
```

ただし `process_rotate180` の **命題自体は TRUE** (全反例形状で計算検証済み)。

### 修正候補 A: r180 固有制約で sortFallingUnits_inversion_is_tied を直接証明

**アイデア**: l_mid と l2 の具体的な構造（r180 由来）を利用し、
sortFallingUnits_inversion_is_tied を一般 Perm でなく r180 固有の Perm に限定して証明。

**必要な変更**:
1. `sortFallingUnits_inversion_is_tied` の仮定に r180 固有制約を追加:
...
3. `sortFallingUnits_foldl_perm_input_eq` の仮定に r180 固有制約を追加
...
問題は sortFallingUnits_inversion_dir_disjoint のコールバックの証明。

**必要な変更**:
1. sortFallingUnits_inversion_dir_disjoint を直接証明（is_tied 経由をスキップ）:
   - r180 由来の Perm では sortFallingUnits 出力間の全反転が direction-disjoint
   - BFS 列挙順序 + r180 変換の層不変性から直接導出
2. shouldProcessBefore_no_chain, sortFallingUnits_spb_order_preserving, sortFallingUnits_inversion_is_tied を削除
...
- shouldProcessBefore_no_mutual, settleStep_comm_ne_dir 等の既存インフラも再利用可能
- 核心タスクは sortFallingUnits_inversion_dir_disjoint の直接証明のみ
- 既存コードの削除量が最小
