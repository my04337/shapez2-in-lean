...
/-- ANEG + ImmBelow → crystal→pin gp mono (4dir, 3L) -/
def checkFull4Dir : IO Unit := do
      ...
      for l1ne in allQ do
       for l1nw in allQ do
        for l1se in allQ do
         for l1sw in allQ do
              let layers := [
                { ne := l0ne, se := l0se, sw := l0sw, nw := l0nw : Layer },
                { ne := l1ne, se := l1se, sw := l1sw, nw := l1nw : Layer }
              ]
...
def main : IO Unit := checkFull4Dir
