...
/-- settled 入力の placeAbove + shatterTopCrystals 後の gravity と rotate180 は可換。
    layerCount ≤ 5 制約なしで成立（IsSettled 入力の構造的性質より）。
    計算検証: 166,757+ shapes (settled 入力, 6L-10L 含む) で 0 failures。
    Plan B-1 (2026-04-14) で axiom 化。波動モデル導入時（Plan B-3）に構成的証明で置換予定。 -/
axiom process_rotate180_placeAbove_settled (bottom top : Shape)
        (h_bottom : bottom.IsSettled) (h_top : top.IsSettled) :
...
/-- stack は rotate180 と可換: 回転してから積層 = 積層してから回転
    ...
    最初の gravity は placeAbove 後のシェイプに対して行われるが、settled 入力の構造的性質より
    layerCount 制約なしで rotate180 等変性が成立する（axiom process_rotate180_placeAbove_settled による）。 -/
theorem stack_rotate180_comm (bottom top : Shape) (config : GameConfig)
    ...
    | some ag =>
        ...
        · -- レイヤ上限内: afterGravity.gravity
          next h_lc =>
            exact Shape.gravity_rotate180_comm ag (Nat.le_trans h_lc h_config)
        · -- レイヤ上限超過: truncate → gravity
          have h_trunc : (ag.truncate config).layerCount ≤ 5 :=
            Nat.le_trans (Shape.truncate_layerCount_le ag config) h_config
          rw [Shape.gravity_rotate180_comm _ h_trunc, Shape.truncate_rotate180]
...
/-- settled 入力の placeAbove + shatterTopCrystals 後の gravity と rotateCW は可換。
    rotate180 版と同様に IsSettled 入力の構造的性質より layerCount 制約なしで成立。
    波動モデル導入時に構成的証明で置換予定。 -/
axiom process_rotateCW_placeAbove_settled (bottom top : Shape)
        (h_bottom : bottom.IsSettled) (h_top : top.IsSettled) :
...
theorem stack_rotateCW_comm (bottom top : Shape) (config : GameConfig)
    ...
    | some ag =>
        ...
        · -- レイヤ上限内: afterGravity.gravity
          next h_lc =>
            exact Shape.gravity_rotateCW_comm ag (Nat.le_trans h_lc h_config)
        · -- レイヤ上限超過: truncate → gravity
          have h_trunc : (ag.truncate config).layerCount ≤ 5 :=
            Nat.le_trans (Shape.truncate_layerCount_le ag config) h_config
          rw [Shape.gravity_rotateCW_comm _ h_trunc, Shape.truncate_rotateCW]
