...
- `S2IL.Kernel.Internal.Rotate180Lemmas` — 書換系（今後整備予定）
-/
...
/-- 空レイヤ判定は CW 回転で保存される。 -/
theorem Layer.isEmpty_rotateCW (l : Layer) :
    l.rotateCW.isEmpty = l.isEmpty := by
  unfold Layer.isEmpty Layer.rotateCW
  cases h0 : (l 0).isEmpty <;>
  cases h1 : (l 1).isEmpty <;>
  cases h2 : (l 2).isEmpty <;>
  cases h3 : (l 3).isEmpty <;>
  simp [Fin.sub_def, h0, h1, h2, h3]

private theorem layerList_dropWhile_map_rotateCW (layers : List Layer) :
    (layers.map Layer.rotateCW).dropWhile Layer.isEmpty =
      (layers.dropWhile Layer.isEmpty).map Layer.rotateCW := by
  induction layers with
  | nil => simp
  | cons l layers ih =>
      by_cases h : l.isEmpty = true
      · have hrot : l.rotateCW.isEmpty = true := by
          simpa [Layer.isEmpty_rotateCW] using h
        simp [List.dropWhile, h, hrot, ih]
      · have hrot : l.rotateCW.isEmpty = false := by
          cases hcase : l.rotateCW.isEmpty
          · rfl
          · have : l.isEmpty = true := by
              simpa [Layer.isEmpty_rotateCW] using hcase
            exact False.elim (h this)
        simp [List.dropWhile, h, hrot]
...
/-- 末尾空レイヤ正規化は CW 回転と可換。 -/
theorem Shape.normalize.rotateCW_comm (s : Shape) :
    (Shape.normalize s).rotateCW = Shape.normalize s.rotateCW := by
  unfold Shape.normalize Shape.dropTrailingEmpty Shape.rotateCW
  rw [List.map_reverse]
  have hReverse : (List.map Layer.rotateCW s).reverse = List.map Layer.rotateCW s.reverse :=
    (List.map_reverse (f := Layer.rotateCW) (l := s)).symm
  rw [hReverse]
  exact congrArg List.reverse (layerList_dropWhile_map_rotateCW s.reverse).symm
...
/-- `QuarterPos.rotateCW` は 4 回で元に戻る。 -/
theorem QuarterPos.rotateCW.four (p : QuarterPos) :
    p.rotateCW.rotateCW.rotateCW.rotateCW = p := by
  obtain ⟨n, d⟩ := p
  apply Prod.ext
  · rfl
  · ext
    simp [QuarterPos.rotateCW, Fin.add_def]
    omega

/-- `down` は CW 回転と可換。 -/
theorem QuarterPos.down_rotateCW (p : QuarterPos) :
    p.down.rotateCW = p.rotateCW.down := by
  rfl
