...
単一チェーン原則（[architecture-layer-ab.md §1.4](../../../docs/s2il/architecture-layer-ab.md)）
に従い、`rotate180` / `rotateCCW` の系を `rotateCW` から 1 行で導出する補助補題を集める。

現時点では `rotate180_eq_rotateCW_rotateCW` 等の基礎系を `Transform.lean`
本体に置いている。今後 `QuarterPos.rotate180` / `Layer.setDir_rotate180_empty`
等の派生系を本ファイルに集約する。
-/
