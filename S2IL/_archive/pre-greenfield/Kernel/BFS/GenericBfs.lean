...
/-- 到達可能ノード上のエッジ保存: seed から到達可能なノード上で edge1 → edge2 が成立すれば、
到達可能性は edge2 に転送される。
非接地ノードの除去（浮遊位置クリア）で接地が保存されることの証明基盤 -/
theorem GenericReachable.edge_preserve_of_reachable
        {edge1 edge2 : α → α → Bool} {s x : α}
        (h_reach : GenericReachable edge1 s x)
        (h : ∀ p q, GenericReachable edge1 s p → edge1 p q = true → edge2 p q = true) :
        GenericReachable edge2 s x := by
    suffices key : ∀ a, GenericReachable edge1 s a → GenericReachable edge1 a x →
        GenericReachable edge2 a x from
        key s .refl h_reach
    intro a ha hax
    induction hax with
    | refl => exact .refl
    | step h_edge h_sub ih =>
        have h_sq : GenericReachable edge1 s _ := ha.trans (.step h_edge .refl)
        exact .step (h _ _ ha h_edge) (ih h_reach h_sq)
...
/-- `genericBfs` は初期 `visited` が `Nodup` なら結果も `Nodup`。
    vis への追加は重複チェック `vis.any (· == pos) = false` の直後にのみ行われるため、
    重複のない集合が不変に保たれる。 -/
theorem genericBfs_nodup [BEq α] [LawfulBEq α] (edge : α → α → Bool)
        (allNodes vis queue : List α) (fuel : Nat) (h_vis : vis.Nodup) :
        (genericBfs edge allNodes vis queue fuel).Nodup := by
    induction fuel generalizing vis queue with
    | zero => exact h_vis
    | succ n ih =>
        cases queue with
        ...
        | cons pos rest =>
            ...
            · rename_i h_not
              ...
              refine ⟨?_, h_vis⟩
              intro h_mem
              have h_any : vis.any (· == pos) = true := by
                  rw [List.any_eq_true]
                  exact ⟨pos, h_mem, BEq.rfl⟩
              exact absurd h_any (by simp [h_not])
