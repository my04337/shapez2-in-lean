...
| 落下処理 (Gravity) | `gravity_IsSettled` | 基盤定理（axiom 化済み、Plan B-1） |
-/
...
private theorem getElem?_dropLast_append_last {α : Type}
        (l : List α) (_h_ne : l ≠ []) (y : α) :
        (l.dropLast ++ [y])[l.length - 1]? = some y := by
    rw [show l.length - 1 = l.dropLast.length from List.length_dropLast.symm]
...
private theorem isGroundingContact_paint (s : Shape) (c : Color) (p1 p2 : QuarterPos) :
    ...
    · rcases hq2 : p2.getQuarter s with _ | q2
      ...
      · obtain ⟨q1', hq1'⟩ := getQuarter_paint_some s c p1 q1 hq1
        ...
        rw [he1, he2]
        -- ピンマッチ部分: ケース展開で解決
        cases q1' <;> cases q1 <;> simp only [Bool.false_eq_true, Bool.true_eq_false] at hp1 <;>
          cases q2' <;> cases q2 <;> simp only [Bool.false_eq_true, Bool.true_eq_false] at hp2 <;> rfl
...
private theorem groundedPositionsList_paint (s : Shape) (c : Color) :
    ...
    have h_edge : Gravity.groundingEdge (s.paint c) = Gravity.groundingEdge s := by
        ext a b; simp only [Gravity.groundingEdge,
            Gravity.isUpwardGroundingContact, isGroundingContact_paint,
            isStructurallyBonded_paint]
    rw [h_edge]
