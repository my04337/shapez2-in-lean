...
- `S2IL.Operations.Gravity.Internal.*` — Wave Gravity の補助 API
- `S2IL.Operations.Shatter.Internal.*` — Shatter primitive の補助 API

今後、各操作ごとの `Operations/<Op>/Internal/` を必要に応じて整備する。
-/
