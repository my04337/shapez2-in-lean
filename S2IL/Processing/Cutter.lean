...
def eastHalf (s : Shape) : Shape :=
    ⟨s.bottom.eastHalf, s.upper.map Layer.eastHalf⟩

/-- シェイプの西側の半分を抽出する。全レイヤの東側象限を空にする -/
def westHalf (s : Shape) : Shape :=
    ⟨s.bottom.westHalf, s.upper.map Layer.westHalf⟩
...
private def zipLayersWithPad (eastLayers westLayers : List Layer) : List Layer :=
    let rec go : List Layer → List Layer → List Layer
        | [], [] => []
        | [], w :: ws => Layer.combineEastWest emptyLayer w :: go [] ws
        | e :: es, [] => Layer.combineEastWest e emptyLayer :: go es []
...
theorem eastHalf_westHalf_combine (s : Shape) :
    ...
    simp [combineHalves, eastHalf, westHalf, layers, zipLayersWithPad]
    sorry
...
/-- 同一シェイプをスワップすると、各出力は元のシェイプと同じ -/
theorem swap_self (s : Shape) :
        s.swap s = (s.settleAfterCut.bind normalize,
                    s.settleAfterCut.bind normalize) := by
    sorry
