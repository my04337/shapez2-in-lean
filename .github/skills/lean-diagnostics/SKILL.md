...
**lean-tooling** の build script 出力を解析し、修正すべき問題を優先順位付けする。
...
|---|---|---|---|
| `unknown identifier '...'` | `lean-proof-methods` スキル → `#loogle` / `#leansearch` | REPL `exact?` | Mathlib/Batteries の名前変更・移動が原因のことが多い |
| `unknown constant '...'` | `grep_search` で定義箇所を検索 | `import` 追加 | ファイル内定義の参照漏れ or import 不足 |
| `type mismatch` | REPL でゴール型を確認 → `norm_cast` / `push_cast` | `lean-proof-methods` スキル | 型変換（Nat↔Int, Fin↔Nat 等）が原因 |
| `unsolved goals` | `lean-theorem-investigator` エージェント | `lean-proof-methods` スキル | ゴール形状を分析して適切なタクティクを選択 |
| `application type mismatch` | 引数の型を REPL `#check` で確認 | `@` で明示適用 | 暗黙引数の推論失敗が多い |
| `function expected` | 括弧・適用の構文を確認 | — | 通常は構文ミス |
| `declaration uses 'sorry'` | `lean-proof-methods` / `s2il-proof-workflow` スキル | — | theorem 化や撤退判断の計画を立てる |
...
エラーメッセージを取得
  ...
  ├─ `unknown identifier` / `unknown constant` → 補題検索（lean-proof-methods）
  ├─ `type mismatch` / `application type mismatch` → 型分析（REPL #check）
  ├─ `unsolved goals` → ゴール分析（lean-theorem-investigator）
  ├─ `sorry` → 証明計画（lean-proof-methods / s2il-proof-workflow）
...
**lean-tooling** / **lean-proof-methods** / **s2il-proof-workflow** / エージェント **lean-build-doctor**（エラー修正候補を含む診断レポート）
