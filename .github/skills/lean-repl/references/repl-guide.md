...
# ファイルから読み込む
Get-Content Scratch/commands-<sessionId>.jsonl | lake exe repl
```
...
.github/skills/lean-repl/scripts/repl.ps1 -NoPickle -InputFile Scratch/commands-<sessionId>.jsonl
```
...
上記を `Scratch/commands-<sessionId>.jsonl` に保存して:
```powershell
.github/skills/lean-repl/scripts/repl.ps1 -Send -SessionId <id> -CmdFile Scratch/commands-<sessionId>.jsonl
```
...
| **`private theorem` が "Unknown identifier" になる** | `private` はモジュール外から不可視。REPL は `import S2IL` した別モジュールとして動作するため、`private` 定義には REPL からアクセスできない。WIP フロー（`docs/agent/protected-wip-workflow.md`）に従い `protected` + `-- [WIP]` に変更してから FQN（`#check @Namespace.lemmaName`）でアクセスする |
