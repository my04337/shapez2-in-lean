...
#
# 使い方:
#   .github/skills/lean-tooling/scripts/repl.sh [--input <path>] [--rebuild-pickle] [--no-pickle]
#
...
build_pickle() {
    ...
    local import_cmd='{"cmd": "import S2IL.Shape\nimport S2IL.Kernel\nimport Plausible"}'
    local pickle_cmd
...
if [[ $NO_PICKLE -eq 0 ]]; then
    ...
    if [[ -n "$INPUT" && -f "$INPUT" ]]; then
        ...
        # ユーザー JSONL がデフォルト import 一式を既に持つ場合だけ重複挿入しない
        if echo "$USER_CONTENT" | grep -q 'import S2IL.Shape' && \
           echo "$USER_CONTENT" | grep -q 'import S2IL.Kernel' && \
           echo "$USER_CONTENT" | grep -q 'import Plausible'; then
        ...
        # インタラクティブモード
        echo "=== Lean REPL 起動（import S2IL.Shape + S2IL.Kernel + Plausible） ===" >&2
