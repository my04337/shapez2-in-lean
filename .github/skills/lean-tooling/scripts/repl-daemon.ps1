...
if ($importResp) {
    try {
        ...
        if ($msgProp) {
            ...
            if ($errs.Count -gt 0) {
                [IO.File]::WriteAllText((Join-Path $sessionDir "error"), "default import failed: $oneLine")
                exit 1
            }
        }
    } catch {
    ...
    [IO.File]::WriteAllText((Join-Path $sessionDir "error"), "default import produced no REPL response")
    exit 1
}
        ...
        if (Test-Path $requestFile) {
            try {
                ...
                foreach ($c in $cmds) {
                    ...
                    if ($obj) {
                        $modified = $false
                        # env 0 is the public imported-environment convention of repl.ps1.
                        # Keep it intact; env > 0 is rebased to the current batch offset.
                        $envProp = $obj.PSObject.Properties['env']
                        if ($envProp) {
                            if ([int]$envProp.Value -gt 0) {
