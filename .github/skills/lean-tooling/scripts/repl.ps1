...
#
# 使い方:
#   .github/skills/lean-tooling/scripts/repl.ps1 [-InputFile <path>] [-RebuildPickle] [-NoPickle]
#
...
#
# デフォルト動作: 安定ベース import（S2IL.Shape + S2IL.Kernel + Plausible）を先頭に自動挿入し env:0 を提供する。
# 操作別 API はコマンド本文で `import S2IL.Operations.<Name>` を明示する。
...
function Build-Pickle {
    ...
    # 安定ベース import をインポートして pickle を保存
    $importCmd  = '{"cmd": "import S2IL.Shape\nimport S2IL.Kernel\nimport Plausible"}'
...
# 安定ベース import を読み込み、env:0 を提供する。
# omega / norm_num / exact? / aesop / plausible など、現行 base で実際に利用できる探索道具を優先する。
function Get-InitCmd {
    return '{"cmd": "import S2IL.Shape\nimport S2IL.Kernel\nimport Plausible"}'
}
...
if ($InputFile -ne "" -and (Test-Path $InputFile)) {
    ...
    # Public convention for repl.ps1 JSONL: env:0 means the imported environment.
    # Keep env:0 intact in default mode; it is valid after the auto import command.
    $normalizedLines = @($lines | ForEach-Object { $_.Trim() })
    $userInput = ($normalizedLines -join "`n`n") + "`n"
}

if (-not $NoPickle) {
    # --- デフォルトモード: 安定ベース import を自動先頭挿入 ---
    # omega / norm_num / exact? / aesop / plausible など現行 base の道具が env:0 で利用可能。
    $initCmd = Get-InitCmd

    if ($null -ne $userInput) {
        # ユーザー JSONL がデフォルト import 一式を既に持つ場合だけ自動先頭挿入をスキップする。
        $hasRoot = $userInput -match 'import\s+S2IL(\\n|\s|")'
        $hasShape = $hasRoot -or ($userInput -match 'import\s+S2IL\.Shape')
        $hasKernel = $hasRoot -or ($userInput -match 'import\s+S2IL\.Kernel')
        $hasPlausible = $userInput -match 'import\s+Plausible'

        if ($hasShape -and $hasKernel -and $hasPlausible) {
            # デフォルト相当のインポートが存在する場合、自動先頭挿入をスキップ
            # ユーザーの import コマンドから "env": N を除去（env 0 はまだ存在しないため）
            $fullInput = $userInput -creplace '("cmd"\s*:\s*"import [^"]+")\s*,\s*"env"\s*:\s*\d+', '$1'
        } else {
            # インポートが不足している場合は自動先頭挿入（操作別 import との重複は無害）
        ...
        # インタラクティブモード
        Write-Host "=== Lean REPL 起動（import S2IL.Shape + S2IL.Kernel + Plausible） ===" -ForegroundColor Cyan
