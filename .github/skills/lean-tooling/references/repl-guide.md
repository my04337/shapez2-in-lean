...
.github/skills/lean-tooling/scripts/repl.ps1 -Send -SessionId <id> -CmdFile Scratch/commands-<sessionId>-<topic>-<runId>.jsonl
.github/skills/lean-tooling/scripts/repl.ps1 -Send -SessionId <id> -Cmd '{"cmd": "#eval 1 + 1"}'
```
...
.github/skills/lean-tooling/scripts/repl.sh --send --session-id <id> --cmd-file Scratch/commands-<sessionId>-<topic>-<runId>.jsonl
.github/skills/lean-tooling/scripts/repl.sh --send --session-id <id> --cmd '{"cmd": "#eval 1 + 1"}'
```
...
.github/skills/lean-tooling/scripts/repl.ps1 -Start -SessionId <id>
# Windows — セッション停止
.github/skills/lean-tooling/scripts/repl.ps1 -Stop -SessionId <id>
```

```bash
# macOS / Linux — セッション開始（import 完了まで待機、初回 ~13s）
.github/skills/lean-tooling/scripts/repl.sh --start --session-id <id>
# macOS / Linux — セッション停止
.github/skills/lean-tooling/scripts/repl.sh --stop --session-id <id>
```
...
.github/skills/lean-tooling/scripts/repl.ps1 -InputFile Scratch/commands-<sessionId>-<topic>-<runId>.jsonl

# macOS / Linux
.github/skills/lean-tooling/scripts/repl.sh --input Scratch/commands-<sessionId>-<topic>-<runId>.jsonl
```
...
REPL スクリプトはデフォルトで `import S2IL.Shape\nimport S2IL.Kernel\nimport Plausible` を先頭コマンドとして自動追加する。
この自動先頭挿入により `env: 0` には Shape/Kernel の安定ベース + Plausible が利用可能な環境が作られる。
`omega` / `norm_num` / `exact?` / `apply?` / `aesop` / `plausible` など、現行 base で実際に使える探索道具を優先する。`ring` / `linarith` / `duper` は default import に含めず、必要な module の build/import を確認してから明示的に使う。

`S2IL` ルート facade と `S2IL.Operations` は自動 import しない。証明作業中に操作モジュールが一時的に壊れても、型・基礎補題・タクティク探索用の REPL を起動しやすくするためである。操作別 API が必要なときは、`env` フィールドなしのコマンド本文に対象 facade を明示する。

```jsonl
{"cmd": "import S2IL.Operations.Gravity\n\nnamespace S2IL\n#check Shape.gravity.of_isSettled\nend S2IL"}
```
...
> ⚠️ **`-NoPickle` の `env:0` は空環境（何もインポートされていない）**。通常モードの `env:0`（安定ベース + `Plausible` 読み込み済）とは**意味が異なる**。JSONL の 1 行目に `import` コマンドを書き、その応答（`env:0`）を以降のコマンドの `"env": 0` として使うこと。

```powershell
# 自動先頭挿入なし（JSONL 内に import 文を書く）
.github/skills/lean-tooling/scripts/repl.ps1 -NoPickle -InputFile Scratch/commands-<sessionId>-<topic>-<runId>.jsonl
```

### pickle（.olean キャッシュ）について

batch モードはデフォルトで `import S2IL.Shape` + `import S2IL.Kernel` + `import Plausible` を直接実行する（pickle は使用しない）。
...
| `import` コマンドを `env` 指定と同時に使うとエラー | `import` を含む JSONL 行は通常の `repl.ps1 -Send/-InputFile` でも `env` なしにする。import 応答の `env` を、以降のコマンドで使う |
| `repl_crashed` / default import failed | JSONL の空行区切りを確認。デフォルト import が失敗した場合は `S2IL.Shape` / `S2IL.Kernel` のビルド状態、操作別 import が失敗した場合は対象 module の build diagnostics を確認する |
...
| **Shape/Operations の名前が "unknown identifier" になる** | デフォルト import は `S2IL.Shape` / `S2IL.Kernel` / `Plausible` まで。操作別 API は `import S2IL.Operations.<Name>` を `env` なしのコマンド本文に書く。名前空間内の宣言は `namespace S2IL ... end` で囲むか、`S2IL.Shape.gravity.of_isSettled` のように FQN を使う |
...
| **`private theorem` が "Unknown identifier" になる** | `private` はモジュール外から不可視。REPL は import した別モジュールとして動作するため、`private` 定義には REPL からアクセスできない。公開補題へ切り出す、または `private` 依存を仮定として持つ `example` で局所検証する |
...
```powershell
# Windows — 主要オプション
.github/skills/lean-tooling/scripts/transplant-proof.ps1 -File S2IL/Foo.lean -Line 42 -ProofFile Scratch/proof.lean
.github/skills/lean-tooling/scripts/transplant-proof.ps1 -File S2IL/Foo.lean -Line 42 -Proof "by omega"
.github/skills/lean-tooling/scripts/transplant-proof.ps1 -File S2IL/Foo.lean -Line 42 -ProofFile Scratch/proof.lean -SimpStabilize
.github/skills/lean-tooling/scripts/transplant-proof.ps1 -File S2IL/Foo.lean -Line 42 -ProofFile Scratch/proof.lean -DryRun
```

```bash
# macOS / Linux
.github/skills/lean-tooling/scripts/transplant-proof.sh --file S2IL/Foo.lean --line 42 --proof-file Scratch/proof.lean
.github/skills/lean-tooling/scripts/transplant-proof.sh --file S2IL/Foo.lean --line 42 --proof "by omega"
.github/skills/lean-tooling/scripts/transplant-proof.sh --file S2IL/Foo.lean --line 42 --proof-file Scratch/proof.lean --simp-stabilize
```
