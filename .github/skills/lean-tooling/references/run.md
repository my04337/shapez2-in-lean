...
> ⚠️ **この参照は `lake exe <target>` 専用**。
> `Scratch/` ファイルを直接実行したい場合は **`lake env lean --run Scratch/MyFile.lean`** を使うこと（`--run` はファイル名より前）。
> 本参照を `Scratch/` 実行に使っても動作しない。

`build.ps1` → `lake exe` の順にビルド＋実行し、出力を検証する。
証明作業や lint チェックが目的なら **build script** で十分。本参照は `Main.lean` の出力確認用。

## スクリプト

```powershell
.github/skills/lean-tooling/scripts/run.ps1          # Windows
.github/skills/lean-tooling/scripts/run.sh            # macOS / Linux
```
