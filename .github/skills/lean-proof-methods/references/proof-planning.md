...
### 環境 preflight（証明作業開始時）

- [ ] REPL で試行錯誤を始める前に `.github/skills/lean-tooling/scripts/build.ps1` を一度実行し、REPL / `.olean` キャッシュを最新化した（局所作業なら `-Target <module>` 可）
- [ ] Lean の判断材料は build script diagnostics / REPL 出力に限定した。VS Code Problems / `get_errors` の Lean 診断は stale / partial state を拾いやすいため基本的に参照しない

### 補題の真偽ゲート（着手前）

- [ ] 型シグネチャ（量化変数・仮定・結論・暗黙引数）を正確に読んだ
- [ ] 反例チェックを実施: `lean-theorem-investigator` エージェント / REPL `by plausible` / `#eval decide`
  ...
  - 層数指針は [`counterexample.md`](counterexample.md) を参照（vanilla5 デフォルト、stress8 は sample cap 付き。vanilla4 は Plausible で使わない）
...
- [ ] 同一 sorry に 8 アプローチ以上失敗 → [`proof-progress.md`](proof-progress.md) で正式撤退判断
...
- `exact?` / `apply?` で見つからない場合は [`mathlib-search.md`](mathlib-search.md) を参照
...
- [`counterexample.md`](counterexample.md) / [`proof-progress.md`](proof-progress.md) 参照
- **lean-theorem-investigator** エージェント（1 件の theorem / sorry / 仮定理に対する反例チェック + タクティク試行 + 補題探索の一括調査）
