...
> 詳細参照: 通常は [`lean-proof-methods`](../SKILL.md) または [`s2il-proof-workflow`](../../s2il-proof-workflow/SKILL.md) を入口にし、詳しい手順が必要なときだけ本ファイルを読む。
...
> 前回との差分比較・ビルド状況・着手推奨までを一括実行できる（内部で lean-build-doctor と lean-theorem-investigator を委任）。
...
[`proof-planning.md`](proof-planning.md) / [`counterexample.md`](counterexample.md) / [`build.md`](../../lean-tooling/references/build.md)
ツール: **s2il-diag**（sorry-list）
エージェント: **lean-build-doctor**（ビルド + sorry インベントリ + エラー修正候補） / **lean-theorem-investigator**（1 件の theorem / sorry 調査） / **lean-session-restorer**（セッション再開）
