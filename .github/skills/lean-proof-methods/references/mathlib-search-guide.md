...
REPL スクリプトが `import S2IL.Shape + S2IL.Kernel + Plausible` を先頭に自動挿入するため、JSONL 内では `"env": 0` を直接使える。追加の Mathlib 検索コマンドは必要な module が使えるか REPL で確認してから使う。
...
- **スキル**: `lean-proof-methods`（検索パイプラインの参照入口）
- **エージェント**: `lean-theorem-investigator`（ゴールから補題を含めた theorem / sorry 調査を自動実行）
...
- **REPL ガイド**: [`repl-guide.md`](../../lean-tooling/references/repl-guide.md) UC-8
