...
> **補題名が不明な場合**: `exact?` で見つからなければ **lean-proof-methods** 参照（または **lean-theorem-investigator** エージェント）で `#leansearch` / `#loogle` を含む段階的検索を実行する。
...
omega, norm_num, exact?, apply?, simp?, aesop, plausible など現行 base で使える tactic を優先する。ring, linarith, nlinarith, duper は必要な module の build/import を確認してから使う。
...
- **lean-theorem-investigator** エージェント — 1 件の theorem / sorry に対してタクティクを自動試行し要約を返す
- [`proof-planning.md`](proof-planning.md) — 証明全体の戦略立案
