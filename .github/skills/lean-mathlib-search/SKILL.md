...
description: >
  Reference catalog for Mathlib/Batteries lemma discovery: `exact?` / `apply?` / `simp?` /
  `#leansearch` / `#loogle` query patterns and a Batteries lemma index.
  Use when: find lemma, search mathlib, exact?, apply?, loogle, leansearch,
  which lemma, lemma search, mathlib lookup, batteries lemma,
  unknown lemma name, find matching theorem,
  補題を探す, 補題名が分からない, どの補題を使う, Mathlib 検索, Batteries 補題,
  補題探索, 補題が見つからない.
  Returns: query templates + ranking heuristics + curated catalog.
  Don't call when: you want the agent to run the searches (use agent `lean-sorry-investigator`).
metadata:
  argument-hint: 'Reference: lemma search query patterns'
---
...
- **エージェント**: `lean-sorry-investigator`（1 件の sorry に対して本スキルの検索手順を自動実行するサブエージェント）
