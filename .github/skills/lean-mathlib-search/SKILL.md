...
description: >
  Lean 4 の証明ゴールに対して Mathlib / Batteries の補題を体系的に検索する。
  exact? / apply? / #leansearch / #loogle を段階的に組み合わせ、ゴールを閉じる補題を発見する。
...
metadata:
  argument-hint: 'ゴール状態またはゴールの説明を渡してください'
---
...
- 存在しない宣言名を `#check` すると REPL がクラッシュする。検索結果から得た名前のみを渡す
...
- **ナレッジ**: [`docs/knowledge/mathlib-batteries-tips.md`](../../../docs/knowledge/mathlib-batteries-tips.md)（`simp`・`Finset` 設計制約等）
