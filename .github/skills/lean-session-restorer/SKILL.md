...
description: >
  Automate Lean 4 proof session restart: diff sorrys, check WIP, recommend next sorry.
...
| `docs/s2il/s2il-lemma-index.md` | 補題インデックス（ファイル構造・sorry 状態・確立済み事実） |
...
前セッションの `protected` + `-- [WIP]` が残存していないかチェックする。
...
| sorry 状態 | アクション |
|---|---|
| sorry 解消済み | **クリーンアップ**: `protected` → `private`、`-- [WIP]` 削除 |
| sorry 未解消 | 作業継続。復元レポートに記載 |
...
- WIP クリーンアップはコード編集を伴うため、レポートで提案のみ行い実際の編集はユーザー判断に任せる
