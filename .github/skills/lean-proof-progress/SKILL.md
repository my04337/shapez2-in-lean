...
2. **証明計画で対象を特定**: [`docs/plans/README.md`](../../../docs/plans/README.md) を入口にして、現在のタスクに対応する計画資料を確認:
   ...
   - シンボル検索は `S2IL/_agent/symbol-map.jsonl` を使用する
3. **sorry 一覧を取得**:
   ```powershell
   ...
   ```
4. **依存関係を把握**: `lake exe s2il-toolkit depgraph --sorry-only --json` で sorry ノードの依存を可視化
...
**lean-proof-planning** / **lean-counterexample** / **lean-build** / **lean-session-restorer**（セッション復元スキル）
