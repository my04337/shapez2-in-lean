...
description: >
  Track Lean 4 sorry status and make retreat decisions across proof sessions.
...
metadata:
  argument-hint: 'Pass sorry name/file or omit for full status'
---
...
> 前回との差分比較・WIP 確認・着手推奨までを一括実行できる。

1. **前回の状態を確認**: `/memories/repo/`、`docs/s2il/`、`/memories/session/`
2. **補題インデックスで対象を特定**: [`docs/s2il/s2il-lemma-index.md`](../../docs/s2il/s2il-lemma-index.md) の以下を確認:
   - 「ファイル構造マップ」→ 対象ファイルの行範囲の特定
   - 「依存関係マップ」→ 証明チェーンの依存順序
   - 「sorry 状態」→ 未証明定理と確立済みの事実
   - これにより Gravity.lean (~6,700行) 等の大規模ファイルでも盲目的な探索を回避できる
3. **sorry 一覧を取得**:
   ```powershell
   ...
   ```
4. **依存関係を把握**: `lean-depgraph` で sorry ノードの依存を可視化
   - `lean-sorry-snapshot` エージェントで Steps 3+4 を一括実行可能
5. **REPL でゴール確認**（オプション）: sorry を含む定理を REPL で実行し `sorries[0].goal` を取得
6. **WIP protected の残存確認**: 前セッションの作業が残っていないかチェック
   ```powershell
   grep_search: query = "-- [WIP]"
   ```
   `-- [WIP]` が見つかった場合: 対応する sorry が解消済みなら**クリーンアップ**（`protected` → `private`、`-- [WIP]` 削除）、sorry が未解消なら作業継続
...
- WIP protected クリーンアップ忘れ: セッション開始時に `grep_search: "-- [WIP]"` で残存確認。sorry 解消済みなのに `protected` のままになっていたら即座にクリーンアップする

## 関連

**lean-proof-planning** / **lean-counterexample** / **lean-build** / **lean-depgraph** / **lean-session-restorer**（セッション復元スキル）
エージェント: **lean-sorry-snapshot** / **lean-theorem-checker** / **lean-session-restorer**（セッション再開時の一括復元）
