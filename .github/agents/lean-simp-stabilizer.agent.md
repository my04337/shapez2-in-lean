...
Build a JSONL command file under `Scratch/commands-simp-stabilizer-<unique>.jsonl` with the theorem fragment up to and including the `simp?` form. Run via `.github/skills/lean-tooling/scripts/repl.ps1 --send --session-id simp-stabilizer-<unique>`.

On Windows, create the JSONL file with a single-line `Set-Content -LiteralPath ... -Encoding UTF8 -Value @('...')` command. Do not use PowerShell here-strings or pipe multi-line snippets into `lake env lean --stdin`; those can leave the terminal in `>>` continuation mode.
...
- If a terminal output shows stale commands or a `>>` prompt, stop using that terminal for verification and restart with a fresh finite `run_in_terminal` command.

## Related

- Skill `lean-simp-guide` — bulk pipeline + caveats
- Skill `lean-tooling` — JSONL execution and Windows terminal safety
