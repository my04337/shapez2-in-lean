---
description: "Run `lake build`, scan diagnostics, and return one report combining (a) error triage with REPL-verified fix candidates and (b) sorry inventory when present. **When**: starting/resuming a session, after a large edit, before committing, or whenever main needs a single source-of-truth view of build health. **Returns**: error table with fix candidates + warning/sorry counts + recommended next 1–3 actions. In `mode=verify-only` returns a single one-liner on success and only escalates to full triage on failure. **Don't call when**: a build was just run in the same turn and main already has the diagnostics in context; for editing one specific simp call (use `lean-simp-stabilizer`); for triaging a theorem target (use `lean-theorem-investigator`)."
...
Run `.github/skills/lean-tooling/scripts/build.ps1` (Windows) or `build.sh` (POSIX) unless a fresh `diagnosticsFile=...` was passed. Extract:
...
| `unsolved goals` | ゴール未解決 | 基本タクティク (`rfl` / `ring` / `omega` / `simp` / `norm_num`) を REPL でバッチ試行。閉じない場合は `lean-theorem-investigator` を main から呼ぶよう推奨 |
...
Windows terminal safety: create REPL JSONL files with single-line `Set-Content -LiteralPath ... -Encoding UTF8 -Value @('...')`; do not send PowerShell here-strings or `lake env lean --stdin` pipelines through `run_in_terminal`. For temporary `.lean` files, use `.github/skills/lean-tooling/scripts/run-lean-file.ps1 -File Scratch/<unique>.lean` and inspect `LEAN_EXIT=<code>`. See skill `lean-tooling` for current command details.
...
Inventory only — **do not generate fix candidates for sorrys**. That is the job of `lean-theorem-investigator`.
...
- Skill `lean-tooling` — build script and REPL command details
- Skill `lean-diagnostics` — error classification routing
- Agent `lean-theorem-investigator` — for in-depth investigation of one theorem target
