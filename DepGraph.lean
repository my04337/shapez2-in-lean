...
structure Config where
    ...
    /-- sorry を持つ宣言のみ表示するか（sorry 間の依存分析に特化） -/
...
def buildGraph (env : Environment) (cfg : Config) :
    ...
    for i in [:moduleNames.size] do
        ...
        for j in [:mdata.constNames.size] do
            ...
            -- private フィルタ:
            -- sorry-only モードでは private 宣言も含める（sorry は private 宣言に存在することが多い）
            if !(cfg.includePrivate || cfg.sorriesOnly) && isPrivateName name then continue
            -- 自動生成された宣言は sorry を含まないため、sorry-only 時も除外する
