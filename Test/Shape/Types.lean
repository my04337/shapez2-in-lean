...
example : (mixedLayer.setDir Direction.ne .empty).getDir Direction.ne = .empty := by
  unfold Layer.getDir Layer.setDir; simp

example : (mixedLayer.setDir Direction.ne .empty).getDir Direction.se =
    mixedLayer.getDir Direction.se := by
...
#guard QuarterPos.layer ((5 : Nat), Direction.sw) == 5
#guard QuarterPos.dir ((5 : Nat), Direction.sw) == Direction.sw
...
example : QuarterPos.getQuarter (QuarterPos.setQuarter s2 (0, Direction.ne) .pin)
    (0, Direction.ne) = .pin := by
  decide

example : QuarterPos.getQuarter (QuarterPos.setQuarter s2 (0, Direction.ne) .pin)
    (0, Direction.se) = QuarterPos.getQuarter s2 (0, Direction.se) := by
  decide

-- 範囲外への setQuarter は無変化
#guard Shape.toString (QuarterPos.setQuarter s2 (99, Direction.ne) .pin) == Shape.toString s2
