...
具体的なシェイプに対する settled / gravity 回帰は
`Test.Operations.Gravity` と `Test.Operations.GravityValidation` で確認する。
-/
...
noncomputable example (s : Shape) : Bool := isSettled s
...
private theorem empty_isSettled : IsSettled ([] : Shape) := by
  intro p hp _
...
example : IsSettled (Shape.rotateCW ([] : Shape)) :=
  IsSettled.rotateCW empty_isSettled
